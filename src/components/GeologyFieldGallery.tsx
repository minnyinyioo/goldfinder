...
type Lang = "zh" | "en" | "my";
const sources = {
  fan: { image: "/images/field-alluvial-fan.jpg", author: "Joseph H. Hartman", license: "CC0", source: "https://commons.wikimedia.org/wiki/File:Alluvial_fan_(GeoDIL_number_-_1366).jpg" },
  terrace: { image: "/images/field-river-terrace.jpg", author: "Bob Forrest", license: "CC BY-SA 2.0", source: "https://commons.wikimedia.org/wiki/File:River_Terrace_-_geograph.org.uk_-_344078.jpg" },
  shear: { image: "/images/field-shear-zone.jpg", author: "GSHD2023", license: "CC BY 4.0", source: "https://commons.wikimedia.org/wiki/File:Mylonitic_shear_zone_B%C3%B6llsteiner_Odenwald.jpg" },
  alteration: { image: "/images/field-hydrothermal-alteration.jpg", author: "James St. John", license: "CC BY 2.0", source: "https://commons.wikimedia.org/wiki/File:Argillic_zone_alteration_%26_hydrothermal_veins_(Orphan_Boy_Mine,_Butte,_Montana,_USA)_1.jpg" },
} as const;
const copy = {
  zh: { eyebrow: "REAL-FIELD REFERENCE", title: "四种关键地貌与构造实拍参照", lead: "先辨认几何形态与空间连续性，再设计对照取样。照片只用于学习特征，不能替代现场比例尺、地层记录与检测。", labels: ["怎么看", "不能证明", "下一步"] as const, source: "原图与许可", cards: [
  ...
  my: { eyebrow: "REAL-FIELD REFERENCE · ကွင်းဆင်းဓာတ်ပုံအစစ်", title: "အရေးကြီး မြေသဏ္ဌာန်နှင့် ဖွဲ့စည်းပုံ လေးမျိုး", lead: "ထိန်းချုပ်နမူနာ မစီမံမီ ပုံသဏ္ဌာန်နှင့် ဆက်လက်တည်ရှိမှုကို စစ်ပါ။ ဓာတ်ပုံသည် လက္ခဏာသင်ယူရန်သာဖြစ်ပြီး field scale၊ section log နှင့် assay ကို အစားမထိုးပါ။", labels: ["ကြည့်ရှုရန်", "သက်သေမပြုနိုင်သည့်အရာ", "နောက်တစ်ဆင့်"] as const, source: "မူရင်းနှင့် licence", cards: [
...
export default function GeologyFieldGallery({ lang }: { lang: Lang }) {
  ...
  return <section className="section geology-field-gallery" id="field-references">
    ...
    <div className="field-reference-grid">{c.cards.map(([key, title, observe, limitation, action]) => { const item = sources[key]; return <article className="field-reference-card" key={key}>
      ...
      <div className="field-reference-copy"><h3>{title}</h3><dl><div><dt><Eye size={16} />{c.labels[0]}</dt><dd>{observe}</dd></div><div><dt><ShieldAlert size={16} />{c.labels[1]}</dt><dd>{limitation}</dd></div><div><dt><FlaskConical size={16} />{c.labels[2]}</dt><dd>{action}</dd></div></dl></div>
    </article>; })}</div>
  </section>;
}
